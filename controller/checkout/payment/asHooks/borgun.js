...
export const useBorgunPayment = ({
  ...
  const process = (billingInfo) => {
    setLoaderTillNextRoute(true);
    console.log('Creating Borgun Payment Order', method);

    const methodRef = { paymentMethod: method };
    beforeOrderCreate(cart, methodRef, billingInfo);
    createOrderMethod({
      variables: {
        ...buildCreateOrderInput(cart, methodRef),
      },
    });
  };
  ...
  return [process, { loading: createOrderLoading, error: createOrderError }];
};
